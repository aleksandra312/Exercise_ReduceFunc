...
function vowelCount(str) {
	return str.split('').reduce(function(accum, nextVal) {
		if (isVowel(nextVal)) {
			if (accum[nextVal]) {
				accum[nextVal.toLowerCase()]++;
			} else {
				accum[nextVal.toLowerCase()] = 1;
			}
		}
		return accum;
	}, {});
...
function addKeyAndValue(arr, key, value) {
	return arr.reduce((accum, nextVal, index) => {
...
the callback function on each value in the array and if the result of the 
callback function at that specific value is true, 
